...
with DAG(
    "37_load_data_orders_to_postgres",
...
""",
    ...
    print_date_orders = SimpleHttpOperator(
        ...
        "Таблица {{ ti.xcom_pull(task_ids='load_orders_data')['table_name'] }}: загружено {{ ti.xcom_pull(task_ids='load_orders_data')['rows_inserted'] }} строк (файл {{ ti.xcom_pull(task_ids='load_orders_data')['file_name'] }}). ",
        method="POST",
        headers={"Content-Type": "application/json"},
        log_response=True,
    )

wait_for_orders >> create_orders_table >> load_orders_task >> print_date_orders
